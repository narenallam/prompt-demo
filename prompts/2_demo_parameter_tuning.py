...
from llm_config import get_llm, get_config
...
def main():
    ...
    for temp in temperatures:
        ...
        response, response_obj, start_time, end_time = llm_temp.invoke_with_metadata(base_prompt)
    ...
    for top_p in top_p_values:
        ...
        response, response_obj, start_time, end_time = llm_top_p.invoke_with_metadata("Generate a list of 5 creative product names for a tech startup.")
        print_token_usage(response_obj, start_time, end_time)
        if response:
            print(response[:200] + "..." if len(response) > 200 else response + "\n")
        else:
    ...
    for max_tokens in token_limits:
        ...
        response, response_obj, start_time, end_time = llm_tokens.invoke_with_metadata("Explain the concept of recursion in programming.")
        print_token_usage(response_obj, start_time, end_time)
        print(response + "\n")
